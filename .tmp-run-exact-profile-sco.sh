...
STRICT_TMP_ROOT=".tmp-head-run-${RUN_DATE}-${TS}"

SOURCE_LOG="$REPORT_DIR/${TS}-leadclaw-source-exact-profile-sco.log"
...
const idx=headers.indexOf('inventory_status');
let count=0;
...
const counts={plumbing:0,hvac:0,roofing:0,other:0,withSitePhone:0};
for(const r of rows){const n=priority(r['Niche']); if(n===0) counts.plumbing++; else if(n===1) counts.hvac++; else if(n===2) counts.roofing++; else counts.other++; if(String(r['Website']||'').trim()&&String(r['Phone']||'').trim()) counts.withSitePhone++;}
...
const out=[headers.join(',')].concat(mapped.map(r=>headers.map(h=>esc(r[h])).join(','))).join('\n')+'\n';
fs.writeFileSync(output,out,'utf8');
const counts={plumbing:0,hvac:0,roofing:0,other:0};
for(const r of mapped){ if(r.__priority===0) counts.plumbing++; else if(r.__priority===1) counts.hvac++; else if(r.__priority===2) counts.roofing++; else counts.other++; }
console.log(JSON.stringify({strictInput:mapped.length,...counts},null,2));
NODE
}
...
let rows=[]; if(fs.existsSync(file) && fs.statSync(file).size>0) rows=parseCSV(fs.readFileSync(file,'utf8')); let counts={Koray:0,Azel:0,Nafim:0};
...
echo "=== SOURCE ===" | tee "$SOURCE_LOG"
RAW_TARGET=1500 LEADCLAW_OUTPUT="$RAW_OUT" node scripts/leadclaw-source.js 2>&1 | tee -a "$SOURCE_LOG"
RAW_SOURCED="$(count_csv_rows "$RAW_OUT")"

echo "=== PRIORITY RAW ORDER ===" | tee -a "$SOURCE_LOG"
priority_sort_raw | tee -a "$SOURCE_LOG"
RAW_WITH_SITE_PHONE="$(node - "$PRIORITY_RAW" <<'NODE'
const fs=require('fs'); const file=process.argv[2];
function splitCSV(line){const out=[];let cur='';let q=false;for(let i=0;i<line.length;i++){const ch=line[i];if(ch==='"'){if(q&&line[i+1]==='"'){cur+='"';i++;}else q=!q;continue;}if(ch===','&&!q){out.push(cur);cur='';continue;}cur+=ch;}out.push(cur);return out;}
if(!fs.existsSync(file)||fs.statSync(file).size===0){console.log(0);process.exit(0)}
const lines=fs.readFileSync(file,'utf8').trim().split(/\r?\n/).filter(Boolean); if(lines.length<=1){console.log(0);process.exit(0)}
const headers=splitCSV(lines[0]); const iWeb=headers.indexOf('Website'); const iPhone=headers.indexOf('Phone'); let c=0; for(const line of lines.slice(1)){const vals=splitCSV(line); if(String(vals[iWeb]||'').trim() && String(vals[iPhone]||'').trim()) c++; } console.log(c);
NODE
)"

echo "=== PREFILTER ===" | tee "$PREFILTER_LOG"
LEADCLAW_INPUT="$PRIORITY_RAW" LEAD_TARGET=1500 node scripts/leadclaw-qualify.js 2>&1 | tee -a "$PREFILTER_LOG"
cp leads/leadclaw/qualified.csv "$PREFILTER_OUT"
PREFILTER_QUALIFIED="$(count_csv_rows "$PREFILTER_OUT")"

echo "=== STRICT INPUT BUILD ===" | tee -a "$PREFILTER_LOG"
build_strict_input | tee -a "$PREFILTER_LOG"
...
STRICT_EXIT=0
echo "=== STRICT ===" | tee "$STRICT_LOG"
set +e
node "$STRICT_TMP_ROOT/scripts/qualify-via-cdp.js" 2>&1 | tee -a "$STRICT_LOG"
...
REPLENISH_EXIT=0
echo "=== REPLENISH ===" | tee "$REPLENISH_LOG"
set +e
QUALIFIED_INPUT=leads/qualified-leads.csv node scripts/replenish-inventory.js 2>&1 | tee -a "$REPLENISH_LOG"
REPLENISH_EXIT=${PIPESTATUS[0]}
set -e
...
if [ "$STRICT_QUALIFIED" -gt 0 ]; then
  if [ "$AVAILABLE_AFTER_REPLENISH" -ge 300 ]; then
    echo "=== DISPATCH ===" | tee "$DISPATCH_LOG"
    set +e
    DISPATCH_RUN_ID="$RUN_LABEL-$RUN_DATE-$TS" node scripts/dispatch-inventory.js 2>&1 | tee -a "$DISPATCH_LOG"
    DISPATCH_EXIT=${PIPESTATUS[0]}
    set -e
    if [ "$DISPATCH_EXIT" -eq 0 ] && [ -f leads/latchly-clean-batch.csv ]; then
      compute_distribution_json "leads/latchly-clean-batch.csv" "$DISTRIBUTION_JSON" | tee -a "$DISPATCH_LOG"
      echo "=== EMAIL ===" | tee "$EMAIL_LOG"
      EMAIL_ATTEMPTED=true
      set +e
      QUALIFIED_INPUT=leads/latchly-clean-batch.csv node scripts/email-setters.js 2>&1 | tee -a "$EMAIL_LOG"
      ...
      if [ "$EMAIL_EXIT" -eq 0 ]; then
        ...
        EMAIL_STATUS="attempted but failed"
        EMAIL_MODE="inventory-dispatch"
      fi
    else
    ...
    compute_distribution_json "leads/qualified-leads.csv" "$DISTRIBUTION_JSON" > /dev/null
    echo "=== EMAIL (PARTIAL DIRECT) ===" | tee "$EMAIL_LOG"
    EMAIL_ATTEMPTED=true
    set +e
    QUALIFIED_INPUT=leads/qualified-leads.csv node scripts/email-setters.js 2>&1 | tee -a "$EMAIL_LOG"
    EMAIL_EXIT=${PIPESTATUS[0]}
    set -e
    if [ "$EMAIL_EXIT" -eq 0 ]; then
      EMAIL_STATUS="sent"
...
$(node - <<'NODE'
const fs=require('fs'); const arr=JSON.parse(fs.readFileSync('leads/tmp/blockers.json','utf8')); if(!arr.length) console.log('- none'); else arr.forEach(x=>console.log(`- ${x}`));
NODE
)
...
cat > "$META_PATH" <<EOF
{
  ...
  "startedAt": $(node -e "process.stdout.write(JSON.stringify(process.argv[1]))" "$TS"),
  ...
  "logs": {
    "source": ${SOURCE_LOG@Q},
    "prefilter": ${PREFILTER_LOG@Q},
    "strict": ${STRICT_LOG@Q},
    "replenish": ${REPLENISH_LOG@Q},
    "dispatch": ${DISPATCH_LOG@Q},
    "email": ${EMAIL_LOG@Q}
  },
...
const summary='leads/reports/2026-04-22-exact-profile-sco-run-summary.md';
const m=JSON.parse(fs.readFileSync(meta,'utf8'));
console.log(JSON.stringify({
  strictQualified:m.strictQualified,
  perSco:m.perSco,
  email:m.email,
  gap:m.gap,
  blockers:m.blockers,
