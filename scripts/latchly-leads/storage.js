...
function createDbStorage(url) {
  ...
  return {
    async init() {
      ...
      await db`ALTER TABLE latchly_leads ADD COLUMN IF NOT EXISTS email_open_count INT NOT NULL DEFAULT 0`;
      await db`ALTER TABLE latchly_leads ADD COLUMN IF NOT EXISTS email_first_opened_at TIMESTAMPTZ`;
      await db`ALTER TABLE latchly_leads ADD COLUMN IF NOT EXISTS email_last_opened_at TIMESTAMPTZ`;
      await db`ALTER TABLE latchly_leads ADD COLUMN IF NOT EXISTS email_click_count INT NOT NULL DEFAULT 0`;
      await db`ALTER TABLE latchly_leads ADD COLUMN IF NOT EXISTS email_first_clicked_at TIMESTAMPTZ`;
      await db`ALTER TABLE latchly_leads ADD COLUMN IF NOT EXISTS email_last_clicked_at TIMESTAMPTZ`;
      await db`ALTER TABLE latchly_leads ADD COLUMN IF NOT EXISTS email_bounced_at TIMESTAMPTZ`;
      await db`ALTER TABLE latchly_leads ADD COLUMN IF NOT EXISTS email_complained_at TIMESTAMPTZ`;
      await db`ALTER TABLE latchly_leads ADD COLUMN IF NOT EXISTS email_replied_at TIMESTAMPTZ`;
      await db`ALTER TABLE latchly_leads ADD COLUMN IF NOT EXISTS email_unsubscribed_at TIMESTAMPTZ`;
      ...
      await db`CREATE INDEX IF NOT EXISTS latchly_engagement_lead_idx ON latchly_lead_engagement_events (lead_id, occurred_at DESC)`;
      await db`CREATE INDEX IF NOT EXISTS latchly_engagement_type_time_idx ON latchly_lead_engagement_events (event_type, occurred_at DESC)`;
